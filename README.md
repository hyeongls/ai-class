...
교차 검증이란 데이터셋을 여러 부분으로 나누고, 이들 부분을 활용하여 모델을 반복적으로 학습하고 평가하는 기법이다.<br/>
K-fold 교차 검증이란 데이터를 K개의 같은 크기의 부분으로 나누는 방법으로 각 부분을 한 번씩 테스트 세트로 사용하고, 나머지 K-1 부분을 학습 세트로 사용하여 모델을 평가하는 방법이다. <br/>

###  20.하이퍼파라미터 튜닝이란 무엇인가?
하이퍼파라미터 튜닝이란 머신러닝 및 딥러닝 모델의 성능을 최적화하기 위해 하이퍼파라미터의 값을 조정하는 과정이다. <br/>
하이퍼파라미터의 종류에는 학습률, 배치 크기(모델 학습 시 사용하는 데이터의 양), 에포크 수, 모델의 구조, 정규화 기법 등이 있다.<br/>
하이퍼파라미터 튜닝 방법에는 그리드 서치, 랜덤 서치, 베이지안 최적화, 자동화된 하이퍼파라미터 튜닝 등이 있다. <br/>

###  21.CNN의 합성곱의 역활은?
CNN에서 합성곱의 역할<br/>
1. 특징 추출 : 입력 데이터와 필터를 통해 중요한 특징을 추출
2. 가중치 공유 : 동일한 필터를 이미지의 모든 영역에 적용하므로, 가중치 공유를 통해 모델의 파라미터 수를 줄인다.
3. 이동 불변성 : 이미지의 위치에 관계없이 동일한 특징을 감지할 수 있도록 도와준다.
4. 차원 축소 : 입력 데이터의 공간적 차원을 줄이는 데 도움을 준다. 여러 개의 필터와 스프라이드를 조정함으로써 입력 이미지의 크기를 줄이면서도 중요한 정보를 유지한다.
5. 특징 맵 생성 : 합성곱 연산의 결과로 생성된 특징 맵은 입력 이미지의 중요한 정보가 포함된 표현이다. 이 특징 맵은 후속 레이어에서 더 깊은 수준의 추출 및 분석을 위해 사용된다.

###  22.CNN의 풀링층의 역활은?
CNN에서 풀링층은 모델의 성능을 향상시키고 계산 비용을 줄이는 데 중요한 역할을 한다.<br/>
1. 차원 축소
2. 특징 요약
3. 이동 불변성 *중요*
4. 계산 비용 감소
...
CNN의 Dense Layer는 모델의 마지막 부분에 위치하며 분류나 회귀 같은 최종 출력 결과를 생성하는데 중요한 역할을 한다. 일반적으로 합성곱 층과 풀링 층에서 추출된 고수준의 특징을 바탕으로 결정을 내리는 역할을 한다.<br/>
1. 고차원 특징을 기반으로 최종 출력 생성
2. 비선형성 추가 및 복잡한 패턴 학습
3. 모델의 학습 가능한 파라미터 증가
4. 특징 결합 및 정보 종합
5. 출력 클래스 결정

###  24.CNN의 stride, filter의 역활? 필터의 가중치는 어떻게 결정되는가?
CNN에서 stride는 합성곱 연산에서 필터가 입력 데이터 위를 얼마나 이동하는지를 나타낸다. stride를 조정해서 특징 추출 위치를 결정하고 출력 크기를 조정한다. <br/> <br/>
CNN에서 Filter는 입력 이미지의 특정 영역과 연산을 수행하는 작은 행렬이다. 일반적으로 필터는 입력 이미지의 패턴, 엣지, 텍스처와 같은 특징을 감지하는 데 사용된다. <br/>
필터의 가중치는 학습을 통해 자동으로 정해지는데 처음엔 랜덤 값으로 초기화 되지만 학습 과정에서 데이터에 맞게 역전파와 경사하강법을 통해 최적화 된다. <br/>

###  25.RNN을 사용하는 이유와 한계점은?
RNN을 사용하는 이유<br/>
1. 순차적 데이터 처리 : 이전 시점의 출력 데이터를 현재 입력 데이터로 사용하기 때문에 데이터의 시간적 순서를 유지하고 시점 간의 관계를 모델링 할 수 있다.
2. 메모리 효과 : RNN은 이전 시점의 정보를 기억하고 다음 시점의 예측에 반영하는 메모리 기능을 가지고 있기 때문에 과거의 정보를 바탕으로 현재 데이터를 더 잘 이해할 수 있다.
3. 변동하는 길이의 데이터 처리 : RNN은 고정된 길이가 아닌, 가변적인 길이의 입력 데이터도 처리할 수 있다. 이 덕에 다양한 길이의 시계열 데이터나 문장 데이터를 처리하는 데 유용하다.
<br/>
RNN의 한계점
1. 장기 의존성 문제 : RNN은 과거 정보를 기억할 수 있지만, 시간이 지남에 따라 초기 시점의 정보가 소실되기 쉽다.
2. 기울기 소실 문제 : 역전파를 통해 학습할 때, 반복되는 미분으로 인해 기울기가 매우 작아져 학습이 이루어지지 않게 된다.
3. 병렬 처리의 어려움 : RNN은 시점 간의 의존성 때문에 병렬 처리가 어렵다. <br/>

###  26.LSTM을 사용하는 이유와 한계점은?
LSTM을 사용하는 이유<br/>
1. 장기 의존성 문제 해결 : RNN의 이전 시점의 정보를 잊어버리는 장기 의존성 문제를 해결하기 위해 게이트 구조를 두어 문제를 해결했다.
2. 게이트 매커니즘을 통한 정보 제어 : 세 가지 게이트(입력 게이트, 망각 게이트, 출력 게이트)를 두어 어떤 정보를 추가하고 잊을지를 결정한다.
<br/><br/>
LSTM의 한계점 <br/>
1. 복잡한 구조와 계산 비용 : LSTM은 게이트 매커니즘과 셀 상태를 유지해야 하므로, 계산 비용이 매우 높다.
2. 장기 메모리의 한계 : RNN보다 장기 의존성을 더 잘 처리하지만, 여전히 매우 긴 시퀀스에서는 성능이 저하될 수 있다. <br/>

###  27.GRU을 사용하는 이유와 차별성은?
GRU를 사용하는 이유<br/>
1. 장기 의존성 문제 해결 : GRU는 RNN에서의 장기 의존성 문제를 해결하기 위해 게이트 매커니즘을 두어 이를 방지한다.
2. 단순하고 효율적인 구조 : GRU는 LSTM에 비해 구조가 더 단순하다. LSTM은 3개의 게이트를 두지만 GRU는 2개의 게이트(업데이트 게이트, 리셋 게이트)만을 사용한다.
3. 빠른 학습 및 더 적은 계산 비용 : LSTM보다 더 단순한 구조덕분에 학습 속도가 빠르며, 더 적은 계산 자원으로도 높은 성능을 발휘한다.
<br/> <br/>
GRU의 차별성 <br/>
1. 구조적 차이
2. 셀 상태 없이 은닉 상태로 정보 전달 : LSTM은 셀 상태와 은닉 상태를 분리하여 정보를 저장하고 전달하지만, GRU는 은닉 상태만을 사용하여 정보를 전달한다.
3. 더 적은 매개변수
4. 비슷한 성능 <br/>

###  28.결정트리에서 불순도(Impurity) – 지니 계수(Gini Index)란 무엇인가?
결정트리에서 불손도란 노드 내의 데이터가 얼마나 혼합되어 있는지를 나타내는 지표이다. 불순도가 높을수록 해당 노드에 서로 다른 클래스의 데이터가 섞여 있다는 의미이다.<br/>
지니 계수란 이러한 불순도를 측정하는 방법 중 하나로 결정트리에서 많이 사용된다. 지니 계수는 노드에 있는 데이터가 얼마나 균일한지 또는 혼합되어 있는지를 평가하며, 값이 0에 가까울수록 해당 노드는 순수하다고 할 수 있다. <br/>

###  29.앙상블이란 무엇인가?
앙상블이란 기계 학습에서 여러 개의 모델을 결합하여 하나의 예측을 수행하는 방법이다. 각각의 개별 모델은 기본 학습기 또는 약한 학습기라고 불리며, 이들의 결합을 통해 보다 정확하고 강건한 예측을 만들 수 있다. <br/>

###  30.부트 스트랩핑(bootstraping)이란 무엇인가?
데이터를 복원 추출하여 여러 번 샘플링한 후, 각 샘플에 대해 모델을 학습시키는 방식으로 주로 배깅 기법에서 사용된다.(중복 허용)<br/>

###  31.배깅(Bagging)이란 무엇인가?
배깅이란 앙상블 학습 기법 중 하나로 부트스프래핑 즉 복원 추출을 통해 여러 개의 훈련 데이터를 생성하는 기법을 기반으로 한다. <br/>

###  32.주성분 분석(PCA) 이란 무엇인가?
PCA란 고차원의 데이터의 차원을 축소하고, 데이터를 시각화하거나 노이즈를 제거하는 데 사용되는 통계적 기법이다.<br/>
PCA의 목적 <br/>
1. 차원 축소
2. 노이즈 제거
3. 데이터 시각화
...
Dense Layer란 인공 신경망에서 가장 기본적이고 널리 사용되는 레이어이다. Dense Layer는 각 입력 노드와 각 출력 노드가 모두 연결되어 있는 완전 연결 층으로 입력 데이터의 모든 특성을 고려하여 출력을 생성한다. <br/>
